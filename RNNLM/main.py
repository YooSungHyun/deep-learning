...
parser.add_argument(
    "--model", type=str, default="LSTM", help="type of network (RNN_TANH, RNN_RELU, LSTM, GRU, Transformer)"
)
...
parser.add_argument(
    "--nhead", type=int, default=2, help="the number of heads in the encoder/decoder of the transformer model"
)
parser.add_argument("--dry-run", action="store_true", help="verify the code and the model")
parser.add_argument("--vocab_path", type=str, default="./vocab.json", help="where is your vocab?")
...
# vocab loading
corpus = data.Corpus(args.vocab_path, args.data)
...
def train(optimizer, scheduler):
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        for p in model.parameters():
            p.data.add_(p.grad, alpha=-cur_lr)
        optimizer.step()
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print(
                "| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.7f} | ms/batch {:5.2f} | "
...
try:
    for epoch in range(1, args.epochs + 1):
        ...
        # learning rate scheduler를 쓰므로, 어닐링은 취소한다.
        # else:
